...
/*   Updated: 2022/11/01 20:00:44 by sanghwal         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (s_a->nodes[s_a->front].data != 0)
		op_ra(s_a, op_lst, 1);
...
void	sort_five(t_deque *s_a, t_deque *s_b, t_oper *op_lst)
{
	...
	while (i <= 5)
	{
		if (s_a->nodes[s_a->front].data <= 1)
			op_pb(s_a, s_b, op_lst);
		else
			op_ra(s_a, op_lst, 1);
		i++;
	}
	if (s_b->nodes[s_b->front].data < s_b->nodes[s_b->rear].data)
		op_sb(s_b, op_lst, 1);
