...
AForm::AForm(void) : name(""), is_signed(false), req_sign(0), req_exe(0) {}
...
void AForm::besigned(Bureaucrat& bur) {
  if (bur.getGrade() <= this->req_sign) {
    std::cout << "[AForm] " << bur.getName() << " signed " << this->name
              << "\n";
    this->is_signed = true;
  } else
...
int AForm::getSignGrade(void) const { return (this->req_sign); }

int AForm::getExeGrade(void) const { return (this->req_exe); }
...
AForm::~AForm() { std::cout << this->name << " [AForm] Destructor called\n"; }
