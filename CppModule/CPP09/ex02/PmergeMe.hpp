...
// 	3. 세 번째 줄에는 첫 번째 컨테이너를 사용하여 정렬하는데 걸린 시간을
// 출력한다.
// 	4. 마지막 줄에는 두 번째 컨테이너를 사용하여 정렬하는데 걸린 시간을
// 출력한다. Ford-Johnson algorithm
// 1. [n/2]개의 서로 다른 원소쌍들을 각 쌍별로 비교한다. (n이 홀수이면 마지막
// 원소는 남겨둔다.)
// 2. 단계1 에서 판정한 더 큰 원소들을 [n/2]개를 병합 삽입으로 정렬한다.
// 3. 주 사슬(b1, a1, a2, a3, ..., a[n/2]), 보조 사슬(b2, b3, b4, ..., b[n/2], b
// > [n/2])
