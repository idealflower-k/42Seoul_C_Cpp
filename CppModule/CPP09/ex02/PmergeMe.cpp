...
void PmergeMe::initData(char** av) {
  ...
  this->_vec.reserve(sizeof(av) / sizeof(av[0]));
...
void PmergeMe::mergeVector(
  ...
  for (size_t i = 1; insertData < pendElemtsSize; ++i) {
    ...
    for (; endIdx <= jacobs;) {
      ...
      std::vector<std::vector<int>::iterator>::iterator first =
          mainChain.begin();
      std::vector<std::vector<int>::iterator>::iterator target =
          std::upper_bound(first, last, pendElemts[jacobs].first,
                           compareVector);
...
std::vector<std::vector<int>::iterator> PmergeMe::makeMainChainVector(
  ...
  mainChain.reserve(pair);
  for (size_t i = 0; i < pair; ++i) {
    if (i == 0)
      target = i * size;
    else
      target = target + (size * 2);
    if (target + (size * 2) > this->_vec.size()) {
      break;
    }
...
PmergeMe::makePendElemtsVector(size_t size) {
  ...
  pendElemts.reserve(this->_vec.size() / size);
  size_t pair = this->_vec.size() / size;
  size_t pending = 0;
  for (size_t i = 1; i < pair; ++i) {
    ...
    if (pending + size > this->_vec.size()) {
      if (pending <= this->_vec.size()) {
...
void PmergeMe::setVectorSortTime() {
  ...
  long secondsDiff = (endTime.tv_sec - this->_vecTime.tv_sec) * 1000000;
...
// fordJohnsonAlgorithmDeque
void PmergeMe::fordJohnsonAlgorithmDeque(size_t size) {
  size_t pair = this->_deq.size() / size;
...
std::deque<std::deque<int>::iterator> PmergeMe::makeMainChainDeque(
  ...
  for (size_t i = 0; i < pair; ++i) {
    if (i == 0)
      ...
      target = target + (size * 2);
    if (target + (size * 2) > this->_deq.size()) {
      break;
    }
...
PmergeMe::makePendElemtsDeque(size_t size) {
  ...
  for (size_t i = 1; i < pair; ++i) {
    if (i == 1)
      ...
      pending = pending + (size * 2);
    if (pending + size > this->_deq.size()) {
      if (pending <= this->_deq.size()) {
        pendElemts.push_back(std::make_pair(
...
void PmergeMe::setDequeSortTime() {
  ...
  long secondsDiff = (endTime.tv_sec - this->_deqTime.tv_sec) * 1000000;
